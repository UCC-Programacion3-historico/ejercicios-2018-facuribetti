...
private:
    ...
    int suma(int N, Nodo<T> *nod){
        if (nod != nullptr){
            if(nod->getDato() > N){
                return nod->getDato() + suma(N,nod->getSiguiente());
            }
            suma(N,nod->getSiguiente());
        }
        return 0;
    }
...
/**
 * Función que devuelve el valor del penultimo elemento
 * @tparam T
 */
template<class T>
T Lista<T>::antePenultimo() {
    T AUX;
    Nodo<T> *aux = inicio;
...
/**
 * Función que mueve el primer elemento a la segunda posicion ---------------------------------------------------------------------------------------------------------
 * @tparam T
 */
template<class T>
void Lista<T>::aSegundo() {
    ...
    if (aux == nullptr)
        throw 404;
    if (aux->getSiguiente() == nullptr) // si no hay segundo elemento (hay uno solo)
        throw 405;

    aMover = inicio; //tengo el Primer elemento en aMover

                                    //El primer elemento es ahora el anterior segundo
    inicio = aux->getSiguiente();   //inicio me apunta al segundo elemento (primero) (4)

//hago que el primer elemento me apunte al tercero (segundo)
    aMover->setSiguiente(inicio->getSiguiente());

    inicio->setSiguiente(aMover);
