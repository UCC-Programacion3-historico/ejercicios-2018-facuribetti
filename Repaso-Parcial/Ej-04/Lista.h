 ...
 */
template<class T>
void Lista<T>::MoverPri(int dato) {

    /* Dia soleado: el dato a mover se encuentra entre las posiciones 1 y n-1.
     * Dia nublado: el dato a mover se encuentra en la primera posicion             X
     * Dia nublado: el dato a mover se encuentra en la ultima posicion.
     * Dia nublado: el dato a mover no existe en la lista dada.                     X
     */
    Nodo<T> *aux = inicio, *mover;

    if (aux == nullptr) //Lista vacia
        throw 404;

    if (aux->getDato() == dato)
        throw 405; // el dato ya esta al inicio

    while(aux->getSiguiente() != nullptr && aux->getSiguiente()->getDato() != dato){ //cuando el dato sea igual o no exista, sale
        aux = aux->getSiguiente();
    }

    if(aux->getSiguiente() == nullptr)
        throw 404; //no existe el dato

        //muevo el dato al principio
        /* Primero tengo que eliminarlo de su posicion original y acomodar esos punteros (el dato anteorior tiene que apuntar
         * al siguiente, como si el dato a mover no existiera), despues tengo que mover el dato a la primera posicion
         * y setear que este dato sea el primero y el siguiente debe ser el que era el primero anteriormente.
         */

        mover = aux->getSiguiente();
        aux->setSiguiente(mover->getSiguiente());

        mover->setSiguiente(inicio);
